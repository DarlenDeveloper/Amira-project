...
// never in the repo. If the file is absent (e.g. a fresh clone or CI without
// secrets), we fall back to debug signing so `flutter run --release` still works.
...
android {
    ...
    buildTypes {
        release {
            ...
            // so local release runs still work.
...
dependencies {
    ...
    // works on older Android API levels. Required when
    // isCoreLibraryDesugaringEnabled = true.
